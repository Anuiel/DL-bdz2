...
def beam_search(
    ...
    top_log_probs = torch.zeros(width, 1)
    ...
    for step in range(max_len - 1):
        ...
        log_prob = torch.nn.functional.log_softmax(model.generator(out[:, -1, :]) / temp, dim=1) * step**length_penalty # (hypotesis_count, vocab_size)
